...
API              	= sfcastapiplatformcustom_api_php_fpm
DATABASE            = sfcastapiplatformcustom_api_db
...
install: build start composer-install yarn-install-front yarn-dev db-reset jwt-key debugoff ## Installation et démarrage du projet
...
bash-api: ## Accéder au bash du serviec API
	$(DOCKER_COMPOSE) exec -w /var/www/sfcastapiplatformcustom $(API) bash

psql:
	$(DOCKER_COMPOSE) exec $(DATABASE) psql --user sfcastapiplatformcustom
...
php-cs-fixer-install:
	$(DOCKER_COMPOSE) exec -w /var/www/sfcastapiplatformcustom -T $(API) composer install
...
php-cs-fixer: ## Lance CS Fixer
	$(DOCKER_COMPOSE) exec -w /var/www/sfcastapiplatformcustom -T $(API) vendor/bin/php-cs-fixer fix --config=.php-cs-fixer.dist.php --dry-run --diff --using-cache=no

php-cs-fixer-fix: ## Lance CS Fixer et fix les fichiers
	$(DOCKER_COMPOSE) exec -w /var/www/sfcastapiplatformcustom -T $(API) vendor/bin/php-cs-fixer fix --config=.php-cs-fixer.dist.php --using-cache=no

php-stan: ## Lance PHP Stan
	$(DOCKER_COMPOSE) exec -w /var/www/sfcastapiplatformcustom -T $(API) ./vendor/bin/phpstan analyse --autoload-file=vendor/autoload.php --no-progress --level 2 -c ./phpstan.neon src
